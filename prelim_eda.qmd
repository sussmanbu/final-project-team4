...
mean_educ <- merged_data |>
  group_by(region, year) |>
  summarise(attend_higher_education = mean(attend_higher_1822_m, na.rm = TRUE)) |>
  ggplot(aes(x = year, y = attend_higher_education, color = region)) +
    geom_line() +
  labs(
...
```{r}
#| label: north africa college
#| echo: false

northern_africa_college <- merged_data |>
  filter(sub_region %in% c('Northern Africa')) |>
  group_by(iso_code) |>
  summarise(mean_primary_completion_rate = mean(comp_prim_v2_m, na.rm = TRUE),
            percent_people_not_in_primary_school = mean(eduout_upsec_m, na.rm = TRUE),
  ...
  ggplot(aes(x = iso_code, y = attend_higher_education, fill = iso_code)) +
    geom_col() +ylim(c(0,1)) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))

northern_africa_college
```

```{r}
#| label: sub-saharan africa college
#| echo: false

saharan_africa_college <- merged_data |>
  filter(sub_region %in% c('Sub-Saharan Africa')) |>
  group_by(iso_code) |>
  summarise(mean_primary_completion_rate = mean(comp_prim_v2_m, na.rm = TRUE),
            percent_people_not_in_primary_school = mean(eduout_upsec_m, na.rm = TRUE),
