...
**The region, Location(rural/urban), the sex, the wealth**, and **comp_prim_v2_m** (the Percentage of primary school completion rate for children and young people aged 3‐5 years above primary school graduation age) can have the effect over **comp_lowsec_v2_m** (the Percentage of lower secondary completion rate for young people aged 3‐5 years above lower secondary school graduation age). Based on the result of the model, every factor can positively affect the model that all their correlation coefficients are positive, which means that as the predictors increase by one unit, the comp_lowsec_v2_m

The R-squared is 0.8525, and the vif shows that there is no collinearity between predictors. And we get the plot for different region's average lower secondary school's completion rate. We found that Africa has the lowest average completion rate, thus, other predictors such as location, sex, wealth and primary completion rate
...
Hypothesis 2: literacy_1524_no \~ comp_prim_1524_no(# of young people aged 15-24 completed primary school) + edu2_2024_no(less than 2 years schooling) + preschool_3_no(# of 3 to 4 year olds attending any type of pre--primary education programme) + region + location + wealth

Pre‐primary education attendance

learning literacy number is for students age group of 15-24.
...
# Transform 'am' into a factor (binary)
Location_data$region <- as.factor(Location_data$region)

# Build the logistic regression model
model <- glm(region ~ literacy_1524_no + comp_lowsec_v2_m, data = Location_data, family = binomial())
